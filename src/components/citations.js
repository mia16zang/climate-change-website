import React from "react";
import Navbar from "./navbar";

function Citations() {
  return (
    <div>
      <div>
        <Navbar />
      </div>
      <div className="bg-white pt-40 px-20">
        <h1 className="text-4xl">Citations</h1>
        <p>
          Climate change and ecosystems: Threats, opportunities and solutions
          ... (n.d.). Retrieved May 4, 2023, from
          https://royalsocietypublishing.org/doi/10.1098/rstb.2019.0104<br></br>{" "}
          Iberdrola. (2021, April 22). Glaciares, Los Grandes Guardianes de la
          Estabilidad del Clima del Planeta. Iberdrola. Retrieved May 4, 2023,
          from
          https://www.iberdrola.com/sustainability/melting-glaciers-causes-effects-solutions
          <br></br>
          Impact of habitat loss on species. WWF. (n.d.). Retrieved May 4, 2023,
          from
          https://wwf.panda.org/discover/our_focus/wildlife_practice/problems/habitat_loss_degradation/{" "}
          <br></br>
          Stock photos, royalty-free images, graphics, vectors &amp; videos.
          Adobe Stock. (n.d.). Retrieved May 4, 2023, from
          https://stock.adobe.com/ <br></br>
          Walther, G.-R., Post, E., Convey, P., Menzel, A., Parmesan, C.,
          Beebee, T. J. C., Fromentin, J.-M., Hoegh-Guldberg, O., &amp;
          Bairlein, F. (n.d.). Ecological responses to recent climate change.
          Nature News. Retrieved May 4, 2023, from
          https://www.nature.com/articles/416389a <br></br>
          World Health Organization. (n.d.). Biodiversity and health. World
          Health Organization. Retrieved May 4, 2023, from
          https://www.who.int/news-room/fact-sheets/detail/biodiversity-and-health{" "}
          <br></br>
          https://www.climatewatchdata.org/data-explorer/historical-emissions?historical-emissions-data-sources=climate-watch&historical-emissions-gases=all-ghg&historical-emissions-regions=All%20Selected&historical-emissions-sectors=total-including-lucf%2Ctotal-including-lucf&page=1
          <br></br>
          https://www.climatewatchdata.org/key-visualizations?visualization=4
          <br></br>
          https://climatedata.imf.org/datasets/4063314923d74187be9596f10d034914_0/explore
          <br></br>
        </p>
      </div>
    </div>
  );
}

export default Citations;
